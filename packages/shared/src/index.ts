export const studioPageIds = ["dashboard", "home", "chat", "hermes", "claude", "sessions", "agents", "codex", "skills", "settings"] as const;
...
export type StudioHostMutationIntent = "root-connect" | "bridge-attach" | "connector-activate" | "connector-lifecycle" | "lane-apply" | "rollback-settlement";
...
export const studioHostBridgeSlotChannels = {
  ...
  rollbackSettlement: "studio:host-executor-slot:rollback-settlement"
} as const;
...
export interface StudioModelMutationResult {
  applied: boolean;
  error: string | null;
  catalog: StudioModelCatalog;
}

export interface StudioGatewayServiceState {
...
export interface StudioGatewayServiceMutationResult {
  applied: boolean;
  error: string | null;
...
export interface StudioOpenClawChatTurnResult {
  prompt: string;
  reply: string;
  source: "runtime" | "mock";
...
export type StudioClaudeMessageRole = "user" | "assistant" | "system" | "tool";
export type StudioClaudeMessageKind = "text" | "tool_use" | "tool_result" | "system" | "error";

export interface StudioClaudeSettings {
  rootPath: string;
  settingsPath: string;
  historyPath: string;
  model: string | null;
...
export interface StudioClaudeSessionSummary {
  id: string;
  projectKey: string;
...
export interface StudioClaudeHistoryEntry {
  id: string;
  display: string;
  timestamp: number;
  projectPath: string | null;
...
export interface StudioClaudeMessage {
  id: string;
...
export interface StudioClaudeSnapshot {
  settings: StudioClaudeSettings;
  sessions: StudioClaudeSessionSummary[];
...
export type StudioHermesAvailability = "connected" | "connecting" | "disconnected" | "blocked";
export type StudioHermesEventLevel = "info" | "warning" | "error";

export interface StudioHermesEvent {
  id: string;
  type: string;
  title: string;
  detail: string;
  timestamp: string;
  level: StudioHermesEventLevel;
  raw?: string;
}

export interface StudioHermesState {
  source: "runtime" | "mock";
  availability: StudioHermesAvailability;
  canConnect: boolean;
  ...
  updatedAt: number | null;
  events: StudioHermesEvent[];
}

export type StudioHermesMessageRole = "user" | "assistant" | "tool" | "system";

export interface StudioHermesMessage {
  id: string;
...
export interface StudioHermesSnapshot {
  state: StudioHermesState;
  sessions: StudioHermesSessionSummary[];
  history: StudioHermesHistoryEntry[];
}

export interface StudioHermesConnectResult {
  started: boolean;
  state: StudioHermesState;
}

export interface StudioHermesDisconnectResult {
  stopped: boolean;
  state: StudioHermesState;
}

export interface StudioHermesSendMessageResult {
  sent: boolean;
  messageId: string | null;
  error: string | null;
}

export interface StudioHermesLoadSessionResult {
  success: boolean;
  messages: StudioHermesMessage[];
  error: string | null;
}

export interface StudioHermesLoadSessionsResult {
  success: boolean;
  sessions: StudioHermesSessionSummary[];
  error: string | null;
}
...
export interface StudioApi {
  ...
  getOpenClawGatewayServiceState(): Promise<StudioGatewayServiceState>;
  startOpenClawGatewayService(): Promise<StudioGatewayServiceMutationResult>;
  stopOpenClawGatewayService(): Promise<StudioGatewayServiceMutationResult>;
  ...
  createHermesSession(modelId?: string | null): Promise<StudioHermesSessionSummary>;
  getHermesGatewayServiceState(): Promise<StudioGatewayServiceState>;
  startHermesGatewayService(): Promise<StudioGatewayServiceMutationResult>;
  stopHermesGatewayService(): Promise<StudioGatewayServiceMutationResult>;
