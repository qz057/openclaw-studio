...
function localizeLaunchTarget(launchTarget) {
  if (process.platform !== "win32" || !launchTarget.startsWith("\\\\")) {
    return launchTarget;
  }

  const localizedRoot = path.join(os.tmpdir(), "openclaw-studio-package-smoke");
  const localizedTarget = path.join(localizedRoot, path.basename(launchTarget));

  fs.rmSync(localizedRoot, { recursive: true, force: true });
  fs.mkdirSync(localizedRoot, { recursive: true });
...
async function main() {
  ...
  const launchTarget = localizeLaunchTarget(packageResult.launchTarget);
