...
function buildStagePackageJson(version, electronVersion) {
  return {
    ...
    dependencies: {
      "@openclaw/shared": "file:node_modules/@openclaw/shared"
    },
    optionalDependencies: electronVersion
      ? {
...
function prepareStage(summary, packagingPaths) {
  ...
  fs.writeFileSync(
    ...
    `${JSON.stringify(buildStagePackageJson(appPackageJson.version, electronVersion), null, 2)}\n`,
    "utf8"
  );
}
...
function packageWindowsLocalTarget(target, options = {}) {
  ...
  patchPackagedSharedDist(paths, packagingPaths);
