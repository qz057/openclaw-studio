...
function renderReleaseNotes({ generatedAt }) {
  return [
    ...
    "- companion route-history memory now feeds a route replay board plus replay scenario packs, a screenshot-driven acceptance review pack, acceptance pass progression, screenshot pass records, a reviewer flow ladder, an acceptance reading queue, a reviewer signoff board, capture review flows, a proof-linked evidence bundle, acceptance evidence continuity, reviewer briefs, proof bundles, product-review console polish, an acceptance storyboard, an evidence dossier, and an evidence trace lens, so returning to publish-gate, approval-queue, rollback-shadow, or handoff relay coverage can restore the last route, sequence, review surface, reviewer walkthrough, signoff verdict, linked proof posture, continuity handoff chain, and multi-window posture locally",
