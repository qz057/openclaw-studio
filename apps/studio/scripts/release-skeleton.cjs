...
function renderReleaseNotes({ generatedAt }) {
  return [
    ...
    "- companion route-history memory now feeds a route replay board plus replay scenario packs, a screenshot-driven acceptance review pack, acceptance pass progression, screenshot pass records, capture review flows, a proof-linked evidence bundle, reviewer briefs, proof bundles, product-review console polish, an acceptance scoreboard, and a replay acceptance checklist, so returning to publish-gate, approval-queue, rollback-shadow, or handoff relay coverage can restore the last route, sequence, review surface, review evidence posture, linked proof posture, and multi-window posture locally",
