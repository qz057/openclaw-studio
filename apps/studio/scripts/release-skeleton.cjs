...
function renderReleaseNotes({ generatedAt }) {
  return [
    ...
    "- companion route-history memory now feeds a route replay board plus replay scenario packs, a screenshot-driven acceptance review pack, an acceptance scoreboard, a pass-status board, a screenshot target board, and a replay acceptance checklist, so returning to publish-gate, approval-queue, rollback-shadow, or handoff relay coverage can restore the last route, sequence, review surface, review evidence posture, and multi-window posture locally",
