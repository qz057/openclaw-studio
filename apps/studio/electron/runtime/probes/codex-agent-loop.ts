import type { SettingItem, StudioStat, StudioTone } from "@openclaw/shared";

type CodexTaskStatus = "running" | "queued" | "needs-review" | "recent" | "complete";

export interface CodexAgentLoopEvent {
  type?: string;
  customType?: string;
  timestamp?: string;
  payload?: {
...
export interface CodexAgentLoopSnapshot {
  state: "stable" | "continuing" | "recovering" | "interrupted" | "complete";
  tone: StudioTone;
  turnCount: number;
...
export interface CodexAgentLoopOverview {
  summary: string;
  stats: StudioStat[];
  signals: SettingItem[];
}
...
function getLabels(event: CodexAgentLoopEvent): string[] {
...
function matchesAny(labels: string[], patterns: RegExp[]): boolean {
  return labels.some((label) => patterns.some((pattern) => pattern.test(label)));
}

function formatStateLabel(state: CodexAgentLoopSnapshot["state"]): string {
  switch (state) {
    case "stable":
      return "Stable";
    case "continuing":
...
function chooseStateTone(state: CodexAgentLoopSnapshot["state"], status: CodexTaskStatus): StudioTone {
  switch (state) {
    case "recovering":
    case "interrupted":
      return "warning";
...
function deriveContinuation(turnCount: number, toolCallCount: number, toolResultCount: number): string {
...
export function analyzeCodexAgentLoop(
  events: CodexAgentLoopEvent[],
  status: CodexTaskStatus,
  updatedAtMs: number,
  now = Date.now()
): CodexAgentLoopSnapshot {
  let turnCount = 0;
  let toolCallCount = 0;
  ...
  const state: CodexAgentLoopSnapshot["state"] = (() => {
    if (status === "complete" || latestCompletionTimestamp > 0 || completionCount > 0) {
      return "complete";
    }
  ...
  const summary =
    `${formatCount(turnCount, "turn")} observed. ${continuation} ` +
...
export function buildCodexAgentLoopOverview(snapshots: CodexAgentLoopSnapshot[]): CodexAgentLoopOverview {
  const primary = snapshots[0] ?? null;

  if (!primary) {
    return {
      summary:
        "No readable live Codex turn history was available, so the shell keeps a typed fallback loop contract that still distinguishes turn state, tool follow-up posture, recovery markers, and interruption handling.",
      stats: [
  ...
  return {
    summary:
      `Read-only agent-loop analysis is now derived from local Codex session logs: ${primary.summary} ` +
