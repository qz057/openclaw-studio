...
export type RuntimeCommandClassification = "read-only" | "workspace-write" | "protected-write" | "dangerous";
export type RuntimeCommandPathScope = "workspace" | "protected" | "system" | "mixed";
export type RuntimeCommandApproval = "not-required" | "required" | "blocked";

export interface RuntimeCommandAssessment {
  command: string;
  classification: RuntimeCommandClassification;
  pathScope: RuntimeCommandPathScope;
  approval: RuntimeCommandApproval;
...
export function matchRuntimeShellRule(permissionRule: string, command: string): boolean {
...
function detectPathScope(command: string): RuntimeCommandPathScope {
...
function classifyCommand(command: string, pathScope: RuntimeCommandPathScope): { classification: RuntimeCommandClassification; reasons: string[] } {
...
export function assessRuntimeCommand(command: string, allowRules: string[] = []): RuntimeCommandAssessment {
  ...
  const approval: RuntimeCommandApproval =
    classification === "dangerous"
      ? "blocked"
...
export function formatRuntimeCommandAssessment(assessment: RuntimeCommandAssessment): string {
  return `${assessment.command} · ${assessment.classification} · ${assessment.pathScope} · approval ${assessment.approval} · rule ${
    assessment.matchedRule ?? "none"
  }`;
}

export function buildRuntimePermissionMatrix(allowRules: string[] = []): RuntimeCommandAssessment[] {
  return [
...
export function buildRuntimeRuleMatchLines(allowRules: string[] = []): string[] {
  ...
  return allowRules.map((rule) => {
    const sample =
      buildRuntimePermissionMatrix(allowRules).find((assessment) => assessment.matchedRule === rule)?.command ?? "no sample match";
