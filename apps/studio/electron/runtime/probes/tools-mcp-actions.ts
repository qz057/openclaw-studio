import type { StudioRuntimeAction } from "@openclaw/shared";

const toolsMcpActionRegistry: Record<string, StudioRuntimeAction[]> = {
...
export function listToolsMcpActions(itemId: string): StudioRuntimeAction[] {
  const actions = toolsMcpActionRegistry[itemId];
  return actions ? actions.map((action) => ({ ...action })) : [];
}

export function getToolsMcpAction(itemId: string, actionId: string): StudioRuntimeAction | null {
