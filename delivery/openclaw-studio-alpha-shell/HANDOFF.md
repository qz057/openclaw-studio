...
`npm run release:plan` 只输出 phase60 review-only delivery chain / release skeleton 汇总，不会写 installer，也不会发布任何 artifact。
...
- command surface 现在具备 action groups / sequences / contextual flows / keyboard routing / action decks / review-surface coverage actions / multi-window review coverage / companion review-path orchestration / sequence-aware companion review navigation / delivery-gate companion sequence switching / companion route-history memory / route replay restore / replay scenario packs / screenshot-driven acceptance review pack / acceptance pass layer / screenshot pass records / capture review flow / proof-linked evidence bundle / acceptance evidence continuity / reviewer brief / proof bundle / product-review console polish / acceptance storyboard / evidence dossier / evidence trace lens / reviewer signoff board / signoff readiness queue / final review closeout / final verdict console / acceptance closeout timeline / final review settlement / pack closeout board / acceptance scoreboard / replay acceptance checks
- Dashboard / Home / Skills 会对当前 route / workflow lane / focused slot 给出一致的 recommended next actions
- inspector 与 windowing 现在补齐 route-aware drilldowns、recent command history、Local Orchestration Board、Final Verdict Console context、Acceptance Closeout Timeline anchor、Review State Continuity / Observability Closeout、以及跨窗口 shared-state continuity readouts
...
- phase60 slice21 进一步补齐 Final Review Closeout、acceptance closeout console、queue / trace / storyboard / dossier / signoff 联动收束、以及当前 replay pack 的最终 reviewer verdict framing
- phase60 slice22 进一步补齐 Pack Closeout Board、pack-level settlement console、signoff queue / acceptance coverage / evidence continuity / final reviewer handoff 联动收束、以及当前 replay pack 的 closeout blocker framing
- phase60 slice23 进一步补齐 Final Review Settlement、verdict progression、linked review surfaces、pack settlement lane、以及当前 replay pack 的 signoff settlement / proof linkage / closeout hierarchy
- phase60 slice24 进一步补齐 Final Verdict Console、reviewer decision context、settlement route anchor、evidence chain、以及当前 replay pack 的 reviewer-facing judgement framing
...
- phase60 slice28 进一步补齐 cross-window shared-state continuity contract、selector-backed continuity matching、Inspector / Windows rail continuity readouts、以及当前 review surface 到 lane / board / mapping 的只读 continuity linkage
- phase60 slice29 进一步补齐 Packaged-app Materialization Contract、Verification manifest / Staged output root / Seal manifest / Integrity manifest readouts、以及当前 delivery stage 到 promotion -> publish gate 的只读 materialization linkage
- phase60 slice30 进一步补齐 Release QA Closeout Readiness、Approval / Audit / Rollback Entry、Stage B / Stage C Bridge、以及当前 delivery stage 到 installer-signing handshake / QA closeout / rollback entry contract 的只读 linkage
- 仍然不做真实外部窗口编排
...
## Phase60 Delivery Skeleton

- `npm run package:alpha`
- 输出到：`delivery/openclaw-studio-alpha-shell`
- 这是 phase60 结构化 alpha-shell snapshot，不是 installer
...
- package 已具备更清晰的 bundle-assembly + packaged-app materialization + packaged-app-directory materialization + packaged-app bundle sealing + packaged-app local materialization contract + release QA closeout readiness + approval-audit-rollback Stage C entry + installer builder execution skeleton + installer channel routing，但仍不是 release publish pipeline，也不是 installer
...
1. 继续在 default-disabled 边界内推进下一刀（优先把 rollback settlement preview contract 从 shared/runtime slot linkage 再推进到 focused-slot / command-surface / review-board alignment），把 execution readiness 从当前 rollback-settlement preview baseline 再推进到更完整的 rollback/apply 协调层
