...
exports.createToolsMcpLocalControlSession = createToolsMcpLocalControlSession;
exports.buildToolsMcpHostExecutorState = buildToolsMcpHostExecutorState;
exports.simulateToolsMcpHostPreviewHandoff = simulateToolsMcpHostPreviewHandoff;
exports.buildToolsMcpShellBoundarySummary = buildToolsMcpShellBoundarySummary;
exports.handoffToolsMcpHostPreview = handoffToolsMcpHostPreview;
exports.probeLiveToolsMcp = probeLiveToolsMcp;
exports.readToolsMcpDetail = readToolsMcpDetail;
exports.runToolsMcpAction = runToolsMcpAction;
...
const codexShellSnapshotsRoot = node_path_1.default.join(codexRoot, "shell_snapshots");
const codexPluginCacheRoot = node_path_1.default.join(codexRoot, "plugins", "cache", "openai-curated");
const codexPluginTempRepoRoot = node_path_1.default.join(codexRoot, ".tmp", "plugins", ".git");
const workspaceToolingRoot = node_path_1.default.join(openclawRoot, "workspace", ".tooling");
const playwrightRunnerRoot = node_path_1.default.join(workspaceToolingRoot, "playwright-runner");
const hooksRoot = node_path_1.default.join(openclawRoot, "workspace", "hooks");
const knownMcpRoots = [
    node_path_1.default.join(codexRoot, "mcp"),
    node_path_1.default.join(codexRoot, "mcp.json"),
    node_path_1.default.join(openclawRoot, "mcp"),
    node_path_1.default.join(openclawRoot, "workspace", ".mcp"),
    node_path_1.default.join(openclawRoot, "workspace", "mcp")
];
function createIdleRootSelectionState() {
    return {
        status: "idle",
        path: null,
        mode: "unresolved",
...
function createIdleBridgeStageState() {
    return {
        status: "idle",
        sourceOrder: [],
        executedAt: null
...
function createIdleConnectorActivationState() {
    return {
        status: "idle",
        label: null,
        mode: "unresolved",
...
function createIdleLaneApplyState() {
    return {
        status: "idle",
        verdict: null,
        rootOverlay: null,
...
function resolvePreconditionState(ready, partial = false) {
    if (ready) {
        return "met";
    }
    if (partial) {
...
function createBoundaryProgression(currentLayer, details) {
    return [
        {
            id: "layer-local-only",
            layer: "local-only",
...
function createBoundarySummary(input) {
    return {
        id: input.id,
        title: input.title,
...
function createHandoffShape(title, summary, fields) {
    return {
        title,
        summary,
        fields
...
function createHostFailureTaxonomy(preferredRootTarget, attachSourceOrder, activationTarget) {
    return [
        {
        ...
        },
        {
            ...
            detail: preferredRootTarget.path !== null || attachSourceOrder.length > 0 || activationTarget.mode !== "unresolved"
...
function createHostLifecycleStages(preferredRootTarget, attachSourceOrder, activationTarget, rootOverlay) {
    return [
        {
            id: "lifecycle-context",
            stage: "collect-context",
            label: "Collect context",
            state: preferredRootTarget.path !== null || attachSourceOrder.length > 0 || activationTarget.mode !== "unresolved" || Boolean(rootOverlay)
                ? "ready"
                : "planned",
            detail: preferredRootTarget.path !== null || attachSourceOrder.length > 0 || activationTarget.mode !== "unresolved" || Boolean(rootOverlay)
...
function createHostRollbackContract(attachSourceOrder, activationTarget, rootOverlay) {
    return {
        status: "planned",
...
function createHostMutationSlots(handoffVersion, preferredRootTarget, attachSourceOrder, activationTarget, rootOverlay) {
    function createSlot(id, intent, label, channel, detail, payloadType, resultType, payloadFields, resultFields) {
        return {
            id,
            intent,
            label,
            ...
            handoff: {
                ...
                payloadType,
                resultType,
                payload: createHandoffShape(`${label} payload`, `Typed payload for ${label.toLowerCase()} now maps preview-host state into the default-disabled phase25 bridge skeleton.`, payloadFields),
                result: createHandoffShape(`${label} result`, `Typed result for ${label.toLowerCase()} now links audit correlation, failure taxonomy, rollback disposition, and simulated outcome trace across the disabled phase25 bridge stub.`, resultFields)
            }
        };
    }
    ...
    return [
        createSlot("slot-root-connect", "root-connect", "Root connect IPC slot", shared_1.studioHostBridgeSlotChannels.rootConnect, preferredRootTarget.path !== null
            ? `Reserved for dedicated-root connect using ${shortenHomePath(preferredRootTarget.path)} (${preferredRootTarget.mode}).`
            : "Reserved for dedicated-root connect, but no root target is currently resolved.", "StudioHostRootConnectPayload", "StudioHostRootConnectResult", [
            ...
            { id: "payload-request-id", label: "Request id", required: true, detail: "Stable host mutation request identifier." },
            {
                ...
                required: true,
                detail: preferredRootTarget.path !== null
        ...
        createSlot("slot-bridge-attach", "bridge-attach", "Bridge attach IPC slot", shared_1.studioHostBridgeSlotChannels.bridgeAttach, attachSourceOrder.length > 0
            ? `Reserved for attach from ${attachSourceOrder.join(" -> ")}.`
            : "Reserved for attach, but no bridge source order is currently resolved.", "StudioHostBridgeAttachPayload", "StudioHostBridgeAttachResult", [
            { id: "payload-preview-id", label: "Preview id", required: true, detail: "Host preview identifier mapped into the slot handoff." },
            { id: "payload-request-id", label: "Request id", required: true, detail: "Stable host mutation request identifier." },
            {
        ...
        createSlot("slot-connector-activate", "connector-activate", "Connector activate IPC slot", shared_1.studioHostBridgeSlotChannels.connectorActivate, `Reserved for lifecycle-aware activation of ${activationTarget.label} (${activationTarget.mode}).`, "StudioHostConnectorActivatePayload", "StudioHostConnectorActivateResult", [
            { id: "payload-preview-id", label: "Preview id", required: true, detail: "Host preview identifier mapped into the slot handoff." },
            { id: "payload-request-id", label: "Request id", required: true, detail: "Stable host mutation request identifier." },
        ...
        createSlot("slot-lifecycle", "connector-lifecycle", "Connector lifecycle IPC slot", shared_1.studioHostBridgeSlotChannels.connectorLifecycle, `Reserved for activate/deactivate/restart lifecycle control of ${activationTarget.label} (${activationTarget.mode}).`, "StudioHostConnectorLifecyclePayload", "StudioHostConnectorLifecycleResult", [
            { id: "payload-preview-id", label: "Preview id", required: true, detail: "Host preview identifier mapped into the slot handoff." },
            { id: "payload-request-id", label: "Request id", required: true, detail: "Stable host mutation request identifier." },
        ...
        createSlot("slot-lane-apply", "lane-apply", "Lane apply IPC slot", shared_1.studioHostBridgeSlotChannels.laneApply, rootOverlay
            ? `Reserved for rollback-aware lane apply using root overlay ${shortenHomePath(rootOverlay)}.`
            : "Reserved for rollback-aware lane apply, but no root overlay is currently resolved.", "StudioHostLaneApplyPayload", "StudioHostLaneApplyResult", [
            { id: "payload-preview-id", label: "Preview id", required: true, detail: "Host preview identifier mapped into the slot handoff." },
            { id: "payload-request-id", label: "Request id", required: true, detail: "Stable host mutation request identifier." },
            {
                id: "payload-root-overlay",
...
function createHostBridgeSimulatedOutcomes(slot) {
    switch (slot.id) {
...
function createHostBridgeState(mutationSlots) {
    ...
    return {
        ...
        summary: "Phase29 registers preview-to-slot mapping, typed validators, disabled Electron slot handlers, detached workspace workflows, readiness-aware window intents, and dock/inspector-friendly placeholder failure paths while host-side execution remains off.",
        mode: "disabled",
        defaultEnabled: false,
...
function buildToolsMcpHostExecutorState(probe, controlSession) {
    ...
    const attachSourceOrder = controlSession.bridgeStage.sourceOrder.length > 0 ? controlSession.bridgeStage.sourceOrder : buildAttachSourceOrder(probe, cachedCuratedPlugins);
    ...
    const mutationSlots = createHostMutationSlots(handoffContractVersion, preferredRootTarget, attachSourceOrder, activationTarget, rootOverlay);
    const bridge = createHostBridgeState(mutationSlots);
    const hostExecutor = {
        id: "host-executor-phase27",
        title: "Disabled host bridge skeleton",
        summary: "Phase55 lowers the executor contract into a default-disabled bridge skeleton with preview-to-slot placeholder handoff, review-only release approval pipeline visibility, dock/inspector linkage, simulated outcomes, and richer approval/audit/rollback traceability while keeping all host mutation disabled.",
        mode: "disabled",
        ...
        intents: [
            ...
            },
            {
                ...
                label: "Bridge attach",
                detail: attachSourceOrder.length > 0
            ...
            deliveryChain: {
                ...
                stageCReadiness: {
                    ...
                    releaseQaCloseoutReadiness: {
                        ...
                        canCloseOut: false,
                        activeTrackId: "",
                        reviewOnlyDeliveryChainPath: "",
                        releaseApprovalWorkflowPath: "",
                        reviewEvidenceCloseoutPath: "",
                        releaseSummaryPath: "",
                    ...
                    approvalWorkflow: {
                        ...
                        gatingHandshakePath: "",
                        approvalBridgePath: "",
                        promotionHandshakePath: "",
                        releaseQaCloseoutReadinessPath: "",
                        approvalAuditRollbackEntryContractPath: "",
                        reviewOnlyDeliveryChainPath: "",
                        operatorReviewBoardPath: "",
                        releaseDecisionHandoffPath: "",
                        reviewEvidenceCloseoutPath: "",
                        blockedBy: [],
                        stages: []
                    },
                    entryContract: {
                        ...
                        reviewOnlyDeliveryChainPath: "",
                        reviewManifestPath: "",
                        attestationApplyAuditPacksPath: "",
                        rollbackLiveReadinessContractsPath: "",
                        rollbackCutoverPublicationReceiptSettlementCloseoutPath: "",
                        checkpoints: [],
                        blockedBy: []
                    },
                    rollbackLiveReadiness: {
                        id: "",
                        label: "",
                        mode: "review-only",
                        summary: "",
...
function createHostPreviewHandoff(preview, hostExecutor, simulation) {
    ...
    return {
        ...
        approval: {
            ...
            summary: simulation.approvalDecision === "approved"
...
function resolveRollbackTraceSummary(handoff, primaryOutcome, terminalOutcome) {
...
function createHostBridgeTraceNotes(handoff, slotHandler, primaryOutcome, terminalOutcome) {
...
function createHostBridgeTrace(handoff, slotHandler, primaryOutcome, terminalOutcome) {
...
function simulateToolsMcpHostPreviewHandoff(handoff, hostExecutor, channel) {
    if (!handoff || handoff.mapping.channel !== channel || handoff.slotResult.channel !== channel) {
        return null;
    }
    const slotHandler = hostExecutor.bridge.slotHandlers.find((handler) => handler.channel === channel && handler.slotId === handoff.mapping.slotId);
...
function resolveHostPreviewForAction(itemId, actionId, probe, controlSession, cachedCuratedPlugins) {
    ...
    switch (`${itemId}:${actionId}`) {
        case "mcp-root-scan:preview-host-root-connect": {
            ...
            const preview = createHostMutationPreview(hostExecutor, "root-connect", "Host root connect preview", "Maps the withheld root-connect preview into the disabled phase25 slot handoff path and simulated placeholder outcome matrix without touching the host runtime.", requestedTarget, preferredRootTarget.path ? "request-approval" : "collect-context");
        ...
        case "mcp-adjacent-runtime:preview-host-bridge-attach": {
            ...
            const preview = createHostMutationPreview(hostExecutor, "bridge-attach", "Host bridge attach preview", "Maps the withheld bridge-attach preview into the disabled phase25 slot handoff path and simulated placeholder outcome matrix without touching the host runtime.", requestedTarget, "handoff-slot");
            const handoff = createHostPreviewHandoff(preview, hostExecutor, {
        ...
        case "mcp-adjacent-runtime:preview-host-connector-activate": {
            const activationTarget = controlSession.rootSelection.status === "selected" && controlSession.rootSelection.path
                ...
                : getConnectorActivationTarget(probe, cachedCuratedPlugins);
            const preview = createHostMutationPreview(hostExecutor, "connector-activate", "Host connector activate preview", "Maps the withheld connector-activate preview into the disabled phase25 slot handoff path and simulated placeholder outcome matrix without touching the host runtime.", activationTarget.label, "mutate-host");
            const handoff = createHostPreviewHandoff(preview, hostExecutor, {
                validationMissingFieldIds: [],
                approvalDecision: "approved",
                auditStatus: "linked",
                ...
                rollbackCheckpoint: activationTarget.mode !== "unresolved" ? `checkpoint:activate:${activationTarget.label}` : "checkpoint:activate-unresolved"
            });
            return {
                hostExecutor,
                preview: {
        ...
        case "mcp-adjacent-runtime:preview-host-lifecycle-rollback": {
            ...
            const localRollbackCouplingReady = controlSession.lifecycleStage.status === "armed" &&
        ...
        case "mcp-adjacent-runtime:preview-host-lane-apply": {
            ...
            const preview = createHostMutationPreview(hostExecutor, "lane-apply", "Host lane apply preview", "Maps the withheld lane-apply preview into the disabled phase25 slot handoff path and simulated placeholder outcome matrix without touching the host runtime.", requestedTarget, "rollback-host");
            const localApplyCouplingReady = controlSession.lifecycleStage.status === "armed" && controlSession.rollbackSettlement.status === "settled";
            const handoff = createHostPreviewHandoff(preview, hostExecutor, {
...
function finalizeHostPreviewResolution(resolution) {
    if (!resolution) {
        return null;
    }
    const simulatedHandoff = simulateToolsMcpHostPreviewHandoff(resolution.handoff, resolution.hostExecutor, resolution.handoff.mapping.channel);
    if (!simulatedHandoff) {
        return resolution;
    }
    return {
...
function createHostBridgeHandoffSections(hostExecutor, hostPreview, hostHandoff) {
    ...
    return [
        ...
        },
        {
            ...
            lines: hostHandoff.simulatedOutcomes.map((outcome) => `${outcome.label} · ${outcome.status} · ${outcome.stage} · ${outcome.failureCode} · rollback ${outcome.rollbackDisposition}`)
        },
        {
...
function formatLocalOnlyStatus(status) {
    switch (status) {
        case "selected":
            return "selected";
        case "staged":
...
function formatSourceOrder(sourceOrder) {
    return sourceOrder.length > 0 ? sourceOrder.join(" -> ") : "none";
}
function recordLocalControlExecution(controlSession, spec) {
...
function resetFromRootSelection(controlSession) {
...
function getHistoryLines(controlSession, itemId) {
    const relevantHistory = controlSession.history.filter((entry) => itemId === "mcp-root-scan" ? entry.itemId === "mcp-root-scan" : entry.itemId === "mcp-adjacent-runtime" || entry.itemId === "mcp-root-scan");
    if (relevantHistory.length === 0) {
        return ["No Studio-local control actions have been executed yet."];
    }
    return relevantHistory.map((entry) => `${entry.executedAt} · ${entry.label} · ${entry.status} · ${entry.target} · ${entry.summary}`);
}
function getRootSessionLines(controlSession) {
    return [
...
function getConnectorSessionLines(controlSession) {
    return [
        ...
        `selected root · ${controlSession.rootSelection.path
        ...
        `connector activation · ${controlSession.connectorActivation.label
...
function getRootDetailSections(controlSession) {
    return [
        {
...
function getConnectorDetailSections(controlSession) {
    return [
        {
...
function getRootDetailTone(controlSession, fallbackTone) {
...
function buildRootStatusLines(scannedRoots, discoveredRoots) {
...
function extractSafeCodexConfigLines(rawConfig) {
    if (!rawConfig) {
        return [];
    }
...
async function describeRootCandidate(rootPath) {
    try {
        const stats = await promises_1.default.stat(rootPath);
        if (stats.isDirectory()) {
            const entries = await promises_1.default.readdir(rootPath);
...
function buildDiscoveryFlowLines(probe) {
    return [
...
function buildConnectorLaneLines(probe) {
    return [
        `curated cache · ${formatPresence(probe.codexPluginCachePresent)}`,
        `plugin installs · ${probe.pluginInstallCount}`,
...
function formatPreviewList(values, formatter = (value) => value, emptyLabel = "none") {
    return values.length > 0 ? values.slice(0, 3).map(formatter).join(" · ") : emptyLabel;
}
function hasConnectorBridgeInputs(probe, cachedCuratedPlugins) {
    return (cachedCuratedPlugins.length > 0 ||
        probe.pluginInstallCount > 0 ||
        probe.pluginEntryCount > 0 ||
        probe.pluginLoadPaths.length > 0 ||
        probe.discoveredMcpRoots.length > 0);
}
function getPreferredRootTarget(probe) {
...
function getConnectorActivationTarget(probe, cachedCuratedPlugins) {
    if (probe.discoveredMcpRoots.length > 0) {
        return {
...
function buildAttachSourceOrder(probe, cachedCuratedPlugins) {
...
function buildRootHostBoundarySummaryLines(probe, controlSession, cachedCuratedPlugins) {
    const preferredRootTarget = getPreferredRootTarget(probe);
    const attachSourceOrder = buildAttachSourceOrder(probe, cachedCuratedPlugins);
    return [
        "future host control · root connect",
        `boundary posture · ${probe.discoveredMcpRoots.length > 0
...
function buildConnectorHostBoundarySummaryLines(probe, controlSession, cachedCuratedPlugins) {
    const attachSourceOrder = controlSession.bridgeStage.sourceOrder.length > 0 ? controlSession.bridgeStage.sourceOrder : buildAttachSourceOrder(probe, cachedCuratedPlugins);
    const activationTarget = controlSession.rootSelection.status === "selected" && controlSession.rootSelection.path
        ? {
            label: shortenHomePath(controlSession.rootSelection.path),
    ...
    return [
        ...
        `boundary posture · ${attachSourceOrder.length >= 2 && activationTarget.mode !== "unresolved"
...
function buildShellBoundarySummary(probe, controlSession) {
    const cachedCuratedPlugins = probe.codexPluginCachePresent ? ["curated-cache"] : [];
    const attachSourceOrder = buildAttachSourceOrder(probe, cachedCuratedPlugins);
    const preferredRootTarget = getPreferredRootTarget(probe);
    const activationTarget = controlSession.rootSelection.status === "selected" && controlSession.rootSelection.path
        ...
        : getConnectorActivationTarget(probe, cachedCuratedPlugins);
    const rootOverlay = controlSession.rootSelection.path ?? preferredRootTarget.path;
    const localControlState = controlSession.laneApply.status === "applied" ||
        controlSession.connectorActivation.status === "active" ||
        controlSession.bridgeStage.status === "staged" ||
    ...
    return createBoundarySummary({
        ...
        summary: "Read-only detail, dry-run planning, Studio-local connector controls, and default-disabled host bridge handoff placeholders are available. Real host execution is still withheld behind policy, approvals, and future executor slots.",
        ...
        policy: {
            ...
            detail: "The shell may inspect runtime state, stage dry-runs, and mutate only the in-memory Studio control session. It cannot write ~/.openclaw, touch installs/config, restart services, or control external processes.",
        ...
        capabilities: [
            ...
            },
            {
                ...
                detail: localControlState === "ready"
...
function buildRootBoundarySummary(probe, controlSession, cachedCuratedPlugins, spec) {
    const preferredRootTarget = getPreferredRootTarget(probe);
    const attachSourceOrder = buildAttachSourceOrder(probe, cachedCuratedPlugins);
    const localRootCapability = controlSession.rootSelection.status === "selected"
        ? "ready"
        : preferredRootTarget.path !== null
            ? "partial"
            : "blocked";
    return createBoundarySummary({
        ...
        policy: {
            ...
            approvalMode: "Explicit host approval required",
            detail: "Root connect may be described and staged locally, but it cannot open a live root connection, write runtime state, or launch connector lifecycle steps in this round.",
        ...
        capabilities: [
            ...
            },
            {
                ...
                detail: controlSession.rootSelection.status === "selected"
...
function buildConnectorBoundarySummary(probe, controlSession, cachedCuratedPlugins, spec) {
    const attachSourceOrder = controlSession.bridgeStage.sourceOrder.length > 0 ? controlSession.bridgeStage.sourceOrder : buildAttachSourceOrder(probe, cachedCuratedPlugins);
    const activationTarget = controlSession.rootSelection.status === "selected" && controlSession.rootSelection.path
        ? {
            label: shortenHomePath(controlSession.rootSelection.path),
    ...
    return createBoundarySummary({
        ...
        blockedReasons: [
            ...
            ...(activationTarget.mode === "unresolved"
                ? [
                    {
                        code: "activation-target-missing",
                        layer: "withheld",
...
function buildToolsMcpShellBoundarySummary(probe, controlSession) {
    return buildShellBoundarySummary(probe, controlSession);
}
async function handoffToolsMcpHostPreview(itemId, actionId, controlSession) {
...
function createLocalExecutionResult(itemId, actionId, controlSession, spec) {
    return {
        itemId,
        actionId,
        title: spec.title,
...
function createHostBoundaryBlockers(blockers) {
    return [
...
function buildHostPermissionBoundaryLines(actionClass, protectedSurfaces) {
    return [
...
function buildHostEnablementLines(extraLines) {
    return [
...
function buildHostExecutorHandoffLines(hostExecutor, hostPreview, hostHandoff) {
    return [
...
function createHostBoundaryResult(itemId, actionId, spec) {
    const bridgeSections = spec.hostPreview && spec.hostHandoff ? createHostBridgeHandoffSections(spec.boundary.hostExecutor, spec.hostPreview, spec.hostHandoff) : [];
    return {
        itemId,
        actionId,
        title: spec.title,
...
function getDetailActions(itemId) {
    return (0, tools_mcp_actions_1.listToolsMcpActions)(itemId);
}
function createActionExecution(action) {
    return {
...
function attachToolsMcpActionContract(result, action) {
    return {
        ...result,
        action,
...
async function probeLiveToolsMcp() {
    const [openclawConfig, codexConfigPresent, codexAuthPresent, codexSessionsPresent, codexShellSnapshotsPresent, codexPluginCachePresent, codexPluginTempRepoPresent, toolingRootPresent, playwrightRunnerPresent, hookNames, discoveredMcpRoots] = await Promise.all([
        ...
        Promise.all(knownMcpRoots.map(async (root) => ((await pathExists(root)) ? root : null))).then((roots) => roots.filter((root) => root !== null))
    ]);
    const hasLiveSignal = Boolean(openclawConfig?.tools) ||
...
async function readToolsMcpDetail(itemId, controlSession) {
    const probe = await probeLiveToolsMcp();
    if (probe.source !== "live") {
        return null;
    }
    const [openclawConfig, codexConfigRaw, toolingPackage, playwrightRunnerPackage, cachedCuratedPlugins] = await Promise.all([
        ...
        readJsonFile(node_path_1.default.join(workspaceToolingRoot, "package.json")),
        readJsonFile(node_path_1.default.join(playwrightRunnerRoot, "package.json")),
    ...
    switch (itemId) {
            ...
            return {
                ...
                sections: [
                    ...
                    },
                    {
                        ...
                        lines: (0, runtime_command_policy_1.buildRuntimePermissionMatrix)(probe.openclawAlsoAllow).map((assessment) => (0, runtime_command_policy_1.formatRuntimeCommandAssessment)(assessment))
                    },
                    {
                        id: "tool-allow-rules",
                        title: "Allow rules",
                        lines: (0, runtime_command_policy_1.buildRuntimeRuleMatchLines)(probe.openclawAlsoAllow)
                    }
                ]
            };
            ...
            return {
                ...
                sections: [
                    ...
                    },
                    {
                        ...
                        lines: cachedCuratedPlugins.length > 0 ? cachedCuratedPlugins.map((entry) => `${entry} · ${shortenHomePath(node_path_1.default.join(codexPluginCacheRoot, entry))}`) : ["No curated plugin cache directories found."]
                    }
                ]
            };
            ...
            return {
                ...
                sections: [
                    ...
                    },
                    {
                        ...
                        lines: probe.hookNames.length > 0 ? probe.hookNames.map((hook) => `${hook} · ${shortenHomePath(node_path_1.default.join(hooksRoot, hook))}`) : ["No workspace hook directories detected."]
                    }
                ]
            };
        case "mcp-root-scan": {
            const rootBoundary = buildRootBoundarySummary(probe, controlSession, cachedCuratedPlugins, {
                ...
                summary: "Root scanning can drive dry-run planning, Studio-local root selection, and preview-host contract visibility, but live host root connect remains withheld.",
                currentLayer: "local-only",
                nextLayer: "preview-host",
            ...
            return {
                ...
                summary: "Known MCP roots were scanned directly; this view shows which paths exist, which remain absent, how the Studio-local root selection session currently stands, and where the blocked host/runtime connect boundary begins.",
                source: "runtime",
                ...
                sections: [
                    ...
                    },
                    {
                        ...
                        lines: probe.discoveredMcpRoots.length > 0
        ...
        case "mcp-adjacent-runtime": {
            const connectorBoundary = buildConnectorBoundarySummary(probe, controlSession, cachedCuratedPlugins, {
                ...
                summary: "Connector bridge attach, activation, and lane apply can be staged locally and previewed structurally, but host-side execution remains withheld behind policy and future executor slots.",
                currentLayer: "local-only",
                nextLayer: "preview-host",
            ...
            return {
                ...
                summary: "Read-only bridge inventory combining Codex plugin cache, OpenClaw plugin installs, load paths, and MCP scan results, plus Studio-local executable control state and an explicit blocked host/runtime boundary layer for attach, activate, and apply flows.",
                source: "runtime",
...
async function runToolsMcpActionInternal(itemId, actionId, controlSession) {
    const probe = await probeLiveToolsMcp();
    if (probe.source !== "live") {
        return null;
    }
    ...
    switch (`${itemId}:${actionId}`) {
        ...
        case "mcp-adjacent-runtime:validate-connector-readiness":
            return {
                ...
                source: "runtime",
                tone: probe.codexPluginCachePresent && (probe.pluginInstallCount > 0 || probe.pluginLoadPaths.length > 0)
                    ? "positive"
        ...
        case "mcp-root-scan:dry-run-connect-root": {
            ...
            return createDryRunResult(itemId, actionId, {
                ...
                outcomeLines: [
                    `predicted result · ${connectReady && preferredRootTarget.path
        ...
        case "mcp-root-scan:execute-local-root-select": {
            const preferredRootTarget = getPreferredRootTarget(probe);
            const selectionStatus = preferredRootTarget.path ? "selected" : "blocked";
        ...
        case "mcp-root-scan:preview-host-root-connect": {
            ...
            const rootHostBoundary = buildRootBoundarySummary(probe, controlSession, cachedCuratedPlugins, {
                ...
                summary: "Preview-host describes the future root connect contract, but live host/runtime root connection remains withheld until approval and executor slots exist.",
            ...
            return createHostBoundaryResult(itemId, actionId, {
                ...
                summary: "Shows the withheld host/runtime root connect path, the current permission boundary, and what would still be required before enabling it.",
                tone: hasDedicatedRoot ? "neutral" : "warning",
                ...
                readinessLines: [
                    ...
                    `supporting source layers · ${attachSourceOrder.length > 0 ? attachSourceOrder.join(" -> ") : "none"}`,
                    `Studio-local selection · ${formatLocalOnlyStatus(controlSession.rootSelection.status)}`,
                    `current readiness · ${hasDedicatedRoot
        ...
        case "mcp-adjacent-runtime:execute-local-bridge-stage": {
            const attachSourceOrder = buildAttachSourceOrder(probe, cachedCuratedPlugins);
            const bridgeStatus = attachSourceOrder.length >= 2 ? "staged" : attachSourceOrder.length === 1 ? "partial" : "blocked";
            ...
            return createLocalExecutionResult(itemId, actionId, controlSession, {
                ...
                executionLines: [
                    ...
                    `selected root overlay · ${controlSession.rootSelection.path ? shortenHomePath(controlSession.rootSelection.path) : "none"}`,
                    `executed at · ${execution.executedAt}`
                ],
                stateLines: [
        ...
        case "mcp-adjacent-runtime:execute-local-connector-activate": {
            ...
            const stagedTarget = controlSession.rootSelection.status === "selected" && controlSession.rootSelection.path
                ...
                : getConnectorActivationTarget(probe, cachedCuratedPlugins);
            const activationStatus = stagedTarget.mode === "unresolved"
                ? "blocked"
                : controlSession.bridgeStage.status === "staged"
                    ? "active"
            ...
            const execution = recordLocalControlExecution(controlSession, {
                ...
                summary: activationStatus === "active"
        ...
        case "mcp-adjacent-runtime:execute-local-lane-apply": {
            ...
            const laneVerdict = laneLines[laneLines.length - 1]?.replace("lane verdict · ", "") ?? "unknown";
            const rootOverlay = controlSession.rootSelection.path ?? preferredRootTarget.path;
            const laneStatus = controlSession.connectorActivation.status === "active" && stagedSourceOrder.length > 0
                ? "applied"
                : controlSession.connectorActivation.status === "prepared" || stagedSourceOrder.length > 0
                    ? "partial"
                    : "blocked";
            const localVerdict = laneStatus === "applied"
        ...
        case "mcp-adjacent-runtime:preview-host-bridge-attach": {
            ...
            const connectorAttachBoundary = buildConnectorBoundarySummary(probe, controlSession, cachedCuratedPlugins, {
                ...
                summary: "Preview-host describes the future bridge attach contract, but live host attach remains withheld until approval, host mutation slots, and rollback semantics exist.",
                currentLayer: "preview-host",
                nextLayer: "future-executor",
            ...
            return createHostBoundaryResult(itemId, actionId, {
                ...
                summary: "Shows the withheld host/runtime bridge attach path, the current permission boundary, and what would be required before enabling it.",
                tone: attachReady ? "neutral" : "warning",
        ...
        case "mcp-adjacent-runtime:preview-host-connector-activate": {
            ...
            const connectorActivateBoundary = buildConnectorBoundarySummary(probe, controlSession, cachedCuratedPlugins, {
                ...
                summary: "Preview-host describes the future connector activation contract, but live host activation remains withheld until approval, lifecycle runner, and executor slots exist.",
                currentLayer: "preview-host",
                nextLayer: "future-executor",
            ...
            return createHostBoundaryResult(itemId, actionId, {
                ...
                summary: "Shows the withheld host/runtime connector activate path, the current permission boundary, and what would be required before enabling it.",
                tone: activationReady ? "neutral" : "warning",
                ...
                readinessLines: [
                    ...
                    `current readiness · ${activationReady
        ...
        case "mcp-adjacent-runtime:preview-host-lifecycle-rollback": {
            ...
            const preferredRootTarget = getPreferredRootTarget(probe);
            const rootOverlay = controlSession.rootSelection.path ?? preferredRootTarget.path;
            const activationReady = controlSession.connectorActivation.status === "active" || controlSession.connectorActivation.status === "prepared";
            ...
            const rollbackBoundary = buildConnectorBoundarySummary(probe, controlSession, cachedCuratedPlugins, {
                ...
                summary: "Preview-host describes rollback coordination across lifecycle and lane apply, but live rollback settlement remains withheld until approval, apply coupling, and executor wiring exist.",
                currentLayer: "preview-host",
                nextLayer: "future-executor",
            ...
            return createHostBoundaryResult(itemId, actionId, {
                ...
                readinessLines: [
                    ...
                    `Studio-local activation state · ${formatLocalOnlyStatus(controlSession.connectorActivation.status)}`,
                    `current readiness · ${rollbackReady
        ...
        case "mcp-adjacent-runtime:preview-host-lane-apply": {
            ...
            const connectorApplyPreviewBoundary = buildConnectorBoundarySummary(probe, controlSession, cachedCuratedPlugins, {
                ...
                summary: "Preview-host describes the future lane apply contract, but live host/runtime apply remains withheld until approval, lifecycle coordination, rollback semantics, and smoke coverage exist.",
                currentLayer: "preview-host",
                nextLayer: "future-executor",
            ...
            return createHostBoundaryResult(itemId, actionId, {
                ...
                summary: "Shows the withheld host/runtime lane apply path, the current permission boundary, and what would be required before enabling it.",
                tone: applyReady ? "neutral" : "warning",
...
async function runToolsMcpAction(itemId, actionId, controlSession) {
    const action = (0, tools_mcp_actions_1.getToolsMcpAction)(itemId, actionId);
    if (!action) {
        return null;
    }
