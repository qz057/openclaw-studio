...
- **phase35/36 formal-release deepening**：
  ...
  - phase60 slice46 继续把 fallback / rollback settlement plan沿 host preview 拉通：把 `precondition-connector-smoke` 升级到 `partial`，把 `rollover` / `rollback missing` blocked reason改成“rollback settlement/apply-coupling disabled”，并新增 `slot-shell-rollback-settlement` + `preview-host-lifecycle-rollback` smoke coverage，目标让 failure-path smoke / rollback settlement contract可以在不执行 host mutation的条件下被验证
