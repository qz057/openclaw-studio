...
- **phase35/36 formal-release deepening**：
  ...
  - phase60 slice36 已继续把 local materialization review packet / validator continuity surface match 推进成更明确的 materialization Stage C readout chain，让 current packet handoff、matched observability surface / continuity spine、以及 nearby QA / approval / entry / rollback posture 在同一张 Stage Explorer 里保持按顺序只读可见，并继续保持 local-only / review-only posture
  - phase60 slice37 已继续把 materialization Stage C readout chain 推进成更明确的 failure-path / command-preview coverage，让 current failure readout、validator-linked rollback anchor、以及 recommended review-surface actions 在同一张 Stage Explorer 里保持按顺序只读可见，并继续保持 local-only / review-only posture
  ...
  - 当前边界内的 Stage C materialization closeout 已完成，全部仍然保持 `local-only`，不做真实 host-side execution
...
1. **local-only**
   - 允许 Studio-local root select / bridge stage / connector activate / lane apply / lifecycle stage / rollback settlement
   - 只改应用内内存状态和执行历史
2. **preview-host**
   - 允许展示 host/runtime root connect / attach / activate / lifecycle / rollback / apply 如果将来开放会怎么执行
   - 显示 blocker、policy、preconditions、future slots
3. **withheld**
   - 当前真实 host-side execution（含 lifecycle / rollback-settlement / apply）明确阻断
4. **future-executor**
   - 未来真实 executor 所需 contract / slot / approval packet / executor packet / route linkage 已命名，但未接线
