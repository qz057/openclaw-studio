...
- 当前核心结论：
  ...
  - phase60 slice51 已继续把 lifecycle/apply coupling 收成更一致的本地状态机：local lifecycle / rollback settlement 现在会显式记录 apply-coupling / recovery-readiness，`preview-host-lifecycle-rollback` / `preview-host-lane-apply` 在未耦合时保持 `invalid + withheld + recovery-drilldown`，耦合完成后切到 `valid + approved + rollback-ready`，且 smoke 已补齐这条失败链 / 就绪链覆盖；仍严格禁止真实 host mutation
